...
void WifiConfigurationAp::StartWebServer()
{
    ...
    httpd_uri_t form_submit = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            if (buf_len > 2048) { // 限制最大请求体大小 (2048: ssid+password+ws_url+long token/JWT)
            ...
            // 可选：保存 Hermes ws 端点与 token 到 "websocket" 命名空间。
            // 使用原生 NVS（与本文件 /advanced/submit 的写法一致），因为本
            // managed component 未链接 main，无法 include settings.h。写入的
            // namespace/keys 与 main/protocols/websocket_protocol.cc 读取的
            // Settings("websocket").GetString("url"/"token") 完全一致。
            cJSON *ws_url_item = cJSON_GetObjectItemCaseSensitive(json, "ws_url");
            cJSON *token_item = cJSON_GetObjectItemCaseSensitive(json, "token");
            bool has_ws_url = cJSON_IsString(ws_url_item) && ws_url_item->valuestring && strlen(ws_url_item->valuestring) > 0;
            bool has_token = cJSON_IsString(token_item) && token_item->valuestring && strlen(token_item->valuestring) > 0;
            if (has_ws_url || has_token) {
                nvs_handle_t ws_nvs;
                esp_err_t ws_err = nvs_open("websocket", NVS_READWRITE, &ws_nvs);
                if (ws_err == ESP_OK) {
                    if (has_ws_url) {
                        nvs_set_str(ws_nvs, "url", ws_url_item->valuestring);
                    }
                    if (has_token) {
                        nvs_set_str(ws_nvs, "token", token_item->valuestring);
                    }
                    nvs_commit(ws_nvs);
                    nvs_close(ws_nvs);
                    ESP_LOGI(TAG, "Saved Hermes websocket config (url=%s, token=%s)",
                        has_ws_url ? "set" : "unset", has_token ? "set" : "unset");
                } else {
                    ESP_LOGE(TAG, "Failed to open websocket NVS: %d", ws_err);
                }
            }
